...
class AirCleanPureController(Node):
    ...
    def set_goal_from_target(self, label: str):
        ...
        self.get_logger().info(
            ...
            f"clean_after_arrival={self.clean_after_arrival}"
        )
        self.publish_debug(
            f"goal {label}: x={x:.2f}, y={y:.2f}, clean={self.clean_after_arrival}"
        )
    ...
    def start_aruco_alignment(self):
        ...
        self.publish_debug(f"aruco alignment started: id={self.aruco_marker_id}")
    ...
    def control_aruco_alignment(self):
        ...
        image_center_x = self.aruco_frame_width / 2.0
        ...
        if abs(error_px) <= self.aruco_center_tolerance_px:
            ...
            self.log_aruco_state(
                f"aruco center stable: error={error_px:.1f}px, "
            ...
            if self.aruco_stable_frames >= self.aruco_finish_required_frames:
                ...
                self.get_logger().info(
                    f"ArUco aligned. Error={error_px:.1f}px. Robot stopped."
                )
                self.publish_debug(f"aruco aligned: error={error_px:.1f}px")
            return

        self.aruco_stable_frames = 0

        normalized_error = error_px / image_center_x
